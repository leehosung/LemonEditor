...
- (BOOL)canAddIU:(NSString *)IUID{
    if(IUID != nil){
        if( [((LMWC *)self.delegate) containsIU:IUID] == NO ){
            return YES;
        }
    }
    return NO;
...
- (BOOL)canRemoveIU:(NSEvent *)theEvent IUID:(NSString *)IUID{
    ...
    if( [((LMWC *)self.delegate) containsIU:IUID] == YES ){
        return NO;
    }
    return YES;
}
            ...
            if ( theEvent.type == NSLeftMouseDown){
                ...
                if (theEvent.clickCount == 1){
                    ...
                    if( [self canRemoveIU:theEvent IUID:currentIUID] ){
                        [((LMWC *)self.delegate) removeSelectedAllIUs];
                        
                    }
                    
                    if([self canAddIU:currentIUID]){
                        [((LMWC *)self.delegate) addSelectedIU:currentIUID];
                    }
